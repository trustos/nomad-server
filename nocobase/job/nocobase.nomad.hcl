...
export PGPASSWORD="{{ key "postgres/adminpassword" }}"
POSTGRES_ADMIN_USER="{{ key "postgres/adminuser" }}"
NOCOBASE_DB_USER="{{ key "nocobase/db_user" }}"
NOCOBASE_DB_PASSWORD="{{ key "nocobase/db_password" }}"
NOCOBASE_DB_NAME="{{ key "nocobase/db_name" }}"
...
EOH
        destination = "local/setup_db.sh"
        perms       = "0755"
      }

      config {
        image = "postgres:16"
        command = "/local/setup_db.sh"
      }
